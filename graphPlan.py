...
def independentPair(a1, a2):
  ...
  if a1 == a2:
    # if it's the same action, it is considered independet from itself (there are no inconsistent effects)
    return True

  for p in a1.getDelete():
    if a2.isPreCond(p) or a2.isPosEffect(p):
      # a1 is deleting a precondition of a2, or a positive effect of a2
      return False

  for p in a2.getDelete():
    if a1.isPreCond(p) or a1.isPosEffect(p):
      # a2 is deleting a precondition of a1, or a positive effect of a1
      return False
